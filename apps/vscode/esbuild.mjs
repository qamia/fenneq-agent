...
const extensionConfig = {
	...
	outfile: `${destDir}/extension-core.js`,
	external: ["vscode", "./launchGate"],
};

// The extension ships as THREE bundles (see src/extension-entry.ts):
//  - extension.js       tiny entry: starts the key gate, then requires the core
//  - launchGate.js      the gate chunk, shared (required) by entry AND core so
//                       its module state exists exactly once
//  - extension-core.js  the full extension — heavy, parsed after the modal shows
const entryConfig = {
	...baseConfig,
	entryPoints: ["src/extension-entry.ts"],
	outfile: `${destDir}/extension.js`,
	external: ["vscode", "./launchGate", "./extension-core.js"],
	plugins: [aliasResolverPlugin, esbuildProblemMatcherPlugin],
};

const gateConfig = {
	...baseConfig,
	entryPoints: ["src/launchGate.ts"],
	outfile: `${destDir}/launchGate.js`,
	external: ["vscode"],
	plugins: [aliasResolverPlugin, esbuildProblemMatcherPlugin],
};
...
async function main() {
	const configs = standalone
		? [standaloneConfig]
		: e2eBuild
			? [e2eBuildConfig]
			: [gateConfig, entryConfig, extensionConfig];
