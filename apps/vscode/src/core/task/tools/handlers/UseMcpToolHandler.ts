...
export class UseMcpToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (
			config.callbacks.shouldAutoApproveTool(block.name) ||
			isToolAutoApproved
		) {
			// Auto-approval flow
			...
			const notificationMessage = `FenneQ wants to use ${tool_name || "unknown tool"} on ${server_name || "unknown server"}`;
		...
		try {
			...
			const notificationsBefore =
				config.services.mcpHub.getPendingNotifications();
			...
			const toolResultToDisplay =
				toolResultText +
				toolResultImages?.map((image: any) => `\n\n${image}`).join("");
