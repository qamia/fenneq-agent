...
export class CondenseHandler implements IToolHandler, IPartialBlockHandler {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "FenneQ wants to condense the conversation...",
				message: `FenneQ is suggesting to condense your conversation with: ${context}`,
			});
		}
		...
		const {
			...
			images,
			files: condenseFiles,
		} = await config.callbacks.ask("condense", context, false);

		// If the user provided a response, treat it as feedback
		if (
			text ||
			(images && images.length > 0) ||
			(condenseFiles && condenseFiles.length > 0)
		) {
	...
	async handlePartialBlock(
		...
		const cleanedContext = uiHelpers.removeClosingTag(
			block,
			"context",
			context,
		);
