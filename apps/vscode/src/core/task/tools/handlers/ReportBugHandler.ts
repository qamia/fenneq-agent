...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	...
	async handlePartialBlock(
		...
		const partialMessage = JSON.stringify({
			...
			what_happened: uiHelpers.removeClosingTag(
				block,
				"what_happened",
				block.params.what_happened,
			),
			steps_to_reproduce: uiHelpers.removeClosingTag(
				block,
				"steps_to_reproduce",
				block.params.steps_to_reproduce,
			),
			api_request_output: uiHelpers.removeClosingTag(
				block,
				"api_request_output",
				block.params.api_request_output,
			),
			additional_context: uiHelpers.removeClosingTag(
				block,
				"additional_context",
				block.params.additional_context,
			),
		});

		await uiHelpers
			.ask(this.name, partialMessage, block.partial)
			.catch(() => {});
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "FenneQ wants to create a github issue...",
				message: `FenneQ is suggesting to create a github issue with the title: ${title}`,
			});
		}
		...
		if (
			...
			(reportBugFiles && reportBugFiles.length > 0)
		) {
			...
			try {
				...
				Logger.error(
					`An error occurred while attempting to report the bug: ${error}`,
				);
			}
