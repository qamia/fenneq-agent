...
function collectPrompts(
	...
	return PROMPT_KEYS.map((key) => block.params[key]?.trim()).filter(
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (didAutoApprove) {
			...
			showNotificationForApproval(
				prompts.length === 1
					? `FenneQ wants to use ${configuredSubagentName ? `the '${configuredSubagentName}' subagent` : "a subagent"}`
					: `FenneQ wants to use ${prompts.length} subagents`,
