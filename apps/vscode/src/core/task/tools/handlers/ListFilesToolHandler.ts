	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!accessValidation.ok) {
			...
			return formatResponse.toolError(
				formatResponse.clineIgnoreError(relDirPath!),
			);
		}
		...
		const workspaceContext = {
			...
			resolutionMethod: (usedWorkspaceHint ? "hint" : "primary_fallback") as
				| "hint"
				| "primary_fallback",
		};
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `FenneQ wants to view directory ${getWorkspaceBasename(absolutePath, "ListFilesToolHandler.notification")}/`;
