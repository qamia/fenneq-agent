...
export class NewTaskHandler implements IToolHandler, IPartialBlockHandler {
	...
	async handlePartialBlock(
		...
		const context = uiHelpers.removeClosingTag(
			block,
			"context",
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "FenneQ wants to start a new task...",
				message: `FenneQ is suggesting to start a new task with: ${context}`,
			});
		}
		...
		const {
			...
			images,
			files: newTaskFiles,
		} = await config.callbacks.ask(this.name, context, false);

		// If the user provided a response, treat it as feedback
		if (
			text ||
			(images && images.length > 0) ||
			(newTaskFiles && newTaskFiles.length > 0)
		) {
