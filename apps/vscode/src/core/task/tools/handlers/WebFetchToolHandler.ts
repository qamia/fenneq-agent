...
export class WebFetchToolHandler implements IFullyManagedTool {
	...
	async handlePartialBlock(
		...
		await uiHelpers
			.ask("tool" as ClineAsk, partialMessage, block.partial)
			.catch(() => {});
	}
		...
		try {
			...
			if (
				...
				return formatResponse.toolError(
					"FenneQ web tools are currently disabled.",
				);
			}
			...
			if (config.callbacks.shouldAutoApproveTool(this.name)) {
				...
				showNotificationForApproval(
					`FenneQ wants to fetch content from ${url}`,
