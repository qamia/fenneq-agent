...
} from "@core/assistant-message/diff";
...
export class WriteToFileToolHandler implements IFullyManagedTool {
	...
	async handlePartialBlock(
		...
		const hasContent =
			block.name === "replace_in_file" ? !!rawDiff : rawContent != null;
			...
			const sharedMessageProps: ClineSayTool = {
				...
				path: getReadablePath(
					...
					uiHelpers.removeClosingTag(
						block,
						block.params.path ? "path" : "absolutePath",
						relPath,
					),
				),
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (block.name === "replace_in_file" && !rawDiff) {
			...
			await config.callbacks.say(
				"error",
				`FenneQ tried to use replace_in_file for '${relPath}' without value for required parameter 'diff'. Retrying...`,
			);
			return formatResponse.toolError(
				formatResponse.replaceInFileMissingDiffError(relPath),
			);
		}
		...
		if (block.name === "write_to_file" && rawContent == null) {
			...
			const lastApiReqTotalTokens = getLastApiReqTotalTokens(
			...
			await config.callbacks.say(
				"error",
				`FenneQ tried to use write_to_file for '${relPath}' without value for required parameter 'content'. ${
					config.taskState.consecutiveMistakeCount >= 2
						? "This has happened multiple times — FenneQ will try a different approach."
						: "Retrying..."
				}`,
			);
		...
		try {
			...
			if (
				...
				const notificationMessage = `FenneQ wants to ${fileExists ? "edit" : "create"} ${getWorkspaceBasename(relPath, "WriteToFile.notification")}`;
	...
	async validateAndPrepareFileOperation(
		...
		const workspaceContext = {
			...
			resolutionMethod: (typeof pathResult !== "string"
				? "hint"
				: "primary_fallback") as "hint" | "primary_fallback",
		};
		...
		if (!accessValidation.ok) {
			...
			const errorResponse = formatResponse.toolError(
				formatResponse.clineIgnoreError(resolvedPath),
			);
		...
		if (config.services.diffViewProvider.editType !== undefined) {
			...
			config.services.diffViewProvider.editType = fileExists
				? "modify"
				: "create";
		}
		...
		if (diff) {
				...
				const errorResponse = formatResponse.toolError(
					`${(error as Error)?.message}\n\n` +
						formatResponse.diffError(
							relPath,
							config.services.diffViewProvider.getOriginalContentForLLM(),
						),
				);
				ToolResultUtils.pushToolResult(
					errorResponse,
			...
			newContent = applyModelContentFixes(
				newContent,
				config.api.getModel().id,
				resolvedPath,
			);
