...
} from "@integrations/misc/extract-file-content";
import {
	arePathsEqual,
	getReadablePath,
...
function getDisplayedLineSlice(
	...
	const requestedEnd =
		endLine !== undefined
			? Math.max(1, endLine)
			: requestedStart + DEFAULT_MAX_LINES - 1;
	const shouldSwapBounds =
		endLine !== undefined && requestedEnd < requestedStart;
	const start = shouldSwapBounds ? requestedEnd : requestedStart;
	const end = Math.min(
		totalLines,
		shouldSwapBounds ? requestedStart : requestedEnd,
	);
...
function parseRequestedLineRange(block: ToolUse): {
	startLine?: number;
	endLine?: number;
} {
	const startLine = block.params.start_line
		? Number.parseInt(block.params.start_line, 10)
		: undefined;
	const endLine = block.params.end_line
		? Number.parseInt(block.params.end_line, 10)
		: undefined;

	return {
		startLine:
			startLine !== undefined && !Number.isNaN(startLine)
				? startLine
				: undefined,
		endLine:
			endLine !== undefined && !Number.isNaN(endLine) ? endLine : undefined,
	};
}
...
export class ReadFileToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `FenneQ wants to read ${getWorkspaceBasename(absolutePath, "ReadFileToolHandler.notification")}`;
		...
		if (validCached) {
			...
			if (validCached.readCount >= 3) {
				await emitReadFileToolUiComplete(
					config,
					sharedMessageProps,
					block,
					fileContent,
