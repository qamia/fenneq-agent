...
import { InspectDataToolHandler } from "./handlers/InspectDataToolHandler";
...
export class ToolExecutorCoordinator {
	...
	private readonly toolHandlersMap: Record<
		ClineDefaultTool,
		(v: ToolValidator) => IToolHandler | undefined
	> = {
		[ClineDefaultTool.ASK]: (_v: ToolValidator) =>
			new AskFollowupQuestionToolHandler(),
		[ClineDefaultTool.ATTEMPT]: (_v: ToolValidator) =>
			new AttemptCompletionHandler(),
		[ClineDefaultTool.BASH]: (v: ToolValidator) =>
			new ExecuteCommandToolHandler(v),
		[ClineDefaultTool.FILE_EDIT]: (v: ToolValidator) =>
			new SharedToolHandler(
				ClineDefaultTool.FILE_EDIT,
				new WriteToFileToolHandler(v),
			),
		[ClineDefaultTool.FILE_READ]: (v: ToolValidator) =>
			new ReadFileToolHandler(v),
		[ClineDefaultTool.FILE_NEW]: (v: ToolValidator) =>
			new WriteToFileToolHandler(v),
		[ClineDefaultTool.SEARCH]: (v: ToolValidator) =>
			new SearchFilesToolHandler(v),
		[ClineDefaultTool.LIST_FILES]: (v: ToolValidator) =>
		...
		[ClineDefaultTool.MCP_ACCESS]: (_v: ToolValidator) =>
			new AccessMcpResourceHandler(),
		[ClineDefaultTool.MCP_DOCS]: (_v: ToolValidator) =>
			new LoadMcpDocumentationHandler(),
		[ClineDefaultTool.NEW_TASK]: (_v: ToolValidator) => new NewTaskHandler(),
		[ClineDefaultTool.PLAN_MODE]: (_v: ToolValidator) =>
			new PlanModeRespondHandler(),
		[ClineDefaultTool.ACT_MODE]: (_v: ToolValidator) =>
			new ActModeRespondHandler(),
		[ClineDefaultTool.TODO]: (_v: ToolValidator) => undefined,
		[ClineDefaultTool.WEB_FETCH]: (_v: ToolValidator) =>
			new WebFetchToolHandler(),
		[ClineDefaultTool.WEB_SEARCH]: (_v: ToolValidator) =>
			new WebSearchToolHandler(),
		[ClineDefaultTool.CONDENSE]: (_v: ToolValidator) => new CondenseHandler(),
		[ClineDefaultTool.SUMMARIZE_TASK]: (_v: ToolValidator) =>
			new SummarizeTaskHandler(_v),
		[ClineDefaultTool.REPORT_BUG]: (_v: ToolValidator) =>
			new ReportBugHandler(),
		[ClineDefaultTool.NEW_RULE]: (v: ToolValidator) =>
			new SharedToolHandler(
				ClineDefaultTool.NEW_RULE,
				new WriteToFileToolHandler(v),
			),
		[ClineDefaultTool.APPLY_PATCH]: (_v: ToolValidator) =>
			new ApplyPatchHandler(_v),
		[ClineDefaultTool.GENERATE_EXPLANATION]: (_v: ToolValidator) =>
			new GenerateExplanationToolHandler(),
		[ClineDefaultTool.USE_SKILL]: (_v: ToolValidator) =>
			new UseSkillToolHandler(),
		[ClineDefaultTool.USE_SUBAGENTS]: (_v: ToolValidator) =>
			new UseSubagentsToolHandler(),
	};
	...
	getHandler(toolName: string): IToolHandler | undefined {
		...
		if (AgentConfigLoader.getInstance().isDynamicSubagentTool(toolName)) {
			...
			const handler = new SharedToolHandler(
				toolName as ClineDefaultTool,
				new UseSubagentsToolHandler(),
			);
