...
export class Task {
	...
	async ask(
		...
		if (
			...
			throw new Error("FenneQ instance aborted");
		}
		let askTs: number;
	...
	private async initiateTaskLoop(userContent: ClineContent[]): Promise<void> {
		...
		while (!this.taskState.abort) {
			...
			// this.say(
			// 	"tool",
			// 	"FenneQ responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
			// )
			nextUserContent = [
				{
					type: "text",
		...
		if (
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message:
						"FenneQ is having trouble. Would you like to continue the task?",
				});
			}
			const { response, text, images, files } = await this.ask(
				"mistake_limit_reached",
				this.api.getModel().id.includes("claude")
					? `This may indicate a failure in FenneQ's thought process or inability to use a tool properly, which can be mitigated with some user guidance (e.g. "Try breaking down the task into smaller steps").`
					: "FenneQ uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, use the Altair tier or higher.",
			);
			...
			*/
			...
			if (assistantHasContent) {
				...
				const baseErrorMessage =
					"Invalid API Response: The provider returned an empty or unparsable response. This is a provider-side issue where the model failed to generate valid output or returned tool calls that FenneQ cannot process. Retrying the request may help resolve this issue.";
