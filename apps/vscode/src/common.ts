...
export async function initialize(
	...
	try {
		...
		Logger.error(
			"[FenneQ] CRITICAL: Failed to initialize StateManager:",
			error,
		);
...
async function showVersionUpdateAnnouncement(stateManager: StateManager) {
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(
				`FenneQ version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`,
			);
			...
			if (lastShownAnnouncementId !== latestAnnouncementId) {
				...
				const message = previousVersion
					? `FenneQ has been updated to v${currentVersion}`
					: `Welcome to FenneQ v${currentVersion}`;
				HostProvider.window.showMessage({
