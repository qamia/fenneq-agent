...
import "should";
import { buildFenneqServerEntry, FENNEQ_SERVER_NAME } from "../defaultServers";
...
describe("fenneqAuth (QAM-498 — per-user workspace auth)", () => {
	...
	async function seedDefault(): Promise<void> {
		// Seeding is URL-gated now (no FENNEQ_MCP_URL in tests), so construct the
		// deployment-configured shape explicitly.
		const settings = {
			mcpServers: {
				[FENNEQ_SERVER_NAME]: buildFenneqServerEntry(
					undefined,
	...
	it("injects the workspace key as a Bearer header on the fenneq server", async () => {
		...
		(await readFenneq()).headers.Authorization.should.equal(
			"Bearer ws-key-123",
		);
	});
