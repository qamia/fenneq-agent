...
export function buildExternalBasicHeaders(): Record<string, string> {
	return {
		"User-Agent": `FenneQ/${ExtensionRegistryInfo.version}`,
	};
}

export async function buildBasicClineHeaders(): Promise<
...
export async function buildClineExtraHeaders(): Promise<
	Record<string, string>
> {
