...
// Qortex curated providers: FenneQ Cloud (Claude tiers) + Moonshot (Kimi, BYOK).
export const FENNEQ_PROVIDERS = ["anthropic", "moonshot"] as const;
...
const ApiOptions = ({
	...
	const providerOptions = useMemo(() => {
		...
		let providers = PROVIDERS.list.filter((option) =>
			(FENNEQ_PROVIDERS as readonly string[]).includes(option.value),
		);
		// Filter by platform
	...
	const handleProviderChange = (newProvider: string) => {
		// biome-ignore lint/suspicious/noExplicitAny: upstream Cline signature
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: searchTerm is the intentional trigger
	useEffect(() => {
		setSelectedIndex(-1);
				...
				<ProviderDropdownWrapper ref={dropdownRef}>
					...
					>
						{searchTerm && searchTerm !== currentProviderLabel && (
							// biome-ignore lint/a11y/noStaticElementInteractions: upstream Cline widget
							// biome-ignore lint/a11y/useAriaPropsSupportedByRole: upstream Cline widget
							// biome-ignore lint/a11y/useKeyWithClickEvents: upstream Cline widget
							<div
